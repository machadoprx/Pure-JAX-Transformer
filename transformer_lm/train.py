	...
	for e in range(epochs):
		...
		for batch, target in tqdm(batched_inputs, total=len(batched_inputs)):
			loss, grads = vmap(jax.value_and_grad(train_step, 1, allow_int=True), in_axes=([0, 0], None, None)) \
																	([batch, target], params, vocab_size)
...
def get_sample_ds(size=2048, seq_len=12, vocab_size=300, bs=8):
	...
	for _ in range(size):
		start = float('inf')
		while start >= seq_len - random_seq - 1:
...
def debug():
	...
	ds = get_sample_ds(size=1024, seq_len=seq_len, vocab_size=vocab_size, bs=bs)
	...
	seq_pred = []
	print(ds[0][0][0], ds[0][1][0])
	in_dec = np.zeros((seq_len), dtype=np.int64)
	in_dec[0] = 300
	print(in_dec)

	print(jnp.argmax(softmax(forward_transformer([ds[0][0][0], in_dec], params, training=False), axis=-1), axis=-1))
