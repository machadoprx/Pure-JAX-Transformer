...
def forward_test(inputs, params):
	...
	for k in range(seq_len):
		...
		out = jnp.matmul(out_dec, params['embed']['W'])
		out_token_id = jnp.argmax(softmax(out, axis=-1), axis=-1)[k]
		out_token_ids.append(out_token_id)
		if out_token_id == 2: # id for EOS
			break

	return jnp.array(out_token_ids)
